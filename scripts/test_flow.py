...
TICKET = a.get("ticket")
d = post("/api/deglare", {"crop": b64(crop, "JPEG", 95), "pad": a["pad"], "ticket": TICKET, "glare_boxes": a.get("glare_boxes_crop", [])})
print("deglare:", {k: v for k, v in d.items() if k != "crop"}); dec(d["crop"]).save(os.path.join(out, f"{tag}_1_deglared.jpg"), quality=95)
e = post("/api/enhance", {"crop": d["crop"], "mode": "faithful", "pad": a["pad"], "ticket": TICKET, "session": "test-" + tag, "consent": True, "meta": a["quality"]})
